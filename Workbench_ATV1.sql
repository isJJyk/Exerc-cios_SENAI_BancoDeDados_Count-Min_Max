...
SELECT MAX(Quantity) FROM OrderDetails GROUP BY ProductID ORDER BY ProductID ASC;
-- 9. Encontre o menor OrderID e ordene por Quantity em ordem decrescente.
SELECT MIN(OrderID) FROM OrderDetails GROUP BY Quantity ORDER BY Quantity DESC;
...
SELECT MAX(Quantity) FROM OrderDetails GROUP BY ProductID ORDER BY ProductID DESC;
...
SELECT MAX(OrderID) FROM orderdetails WHERE CAST(ProductID AS CHAR) LIKE '2%';
...
SELECT MIN(Quantity) FROM orderdetails WHERE CAST(ProductID AS CHAR) LIKE '%5';
